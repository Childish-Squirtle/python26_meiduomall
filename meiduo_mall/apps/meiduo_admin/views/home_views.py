...
class HomeView(ViewSet):
    ...
    def day_active(self, request):
        ...
        count = User.objects.filter(last_login__gte= shanghai_0_utc).count()
        #3.构建响应对象
        return Response({
            'count': count,
            'date': cur_date.date()
    ...
    def day_orders(self, request):
        ...
        #从主表入手查询用户数量
        user_list = User.objects.filter(orders__create_time__gte= shanghai_0_utc)#queryset集合，主表对象没有去重
        #去重
        count = len(set(user_list))
